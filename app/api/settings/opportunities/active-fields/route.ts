...
export async function GET() {
  try {
    ...
    const defaultFields = [
      ...
      { name: "contact_email", type: "email", label: "Email", required: true, visible: true },
      { name: "contact_phone", type: "text", label: "Teléfono", required: false, visible: true },
      { name: "company", type: "text", label: "Empresa", required: true, visible: true },
    ...
    console.error("Error fetching opportunities active fields:", error)
    return NextResponse.json(
      { success: false, error: "Error al obtener campos activos de oportunidades" },
      { status: 500 },
    )
  }
}
