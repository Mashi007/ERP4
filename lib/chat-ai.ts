...
PREGUNTA DEL USUARIO: ${message}
    ...
    if (!hasClaudeAPI && !process.env.XAI_API_KEY) {
      // Respuesta básica sin IA
      return generateBasicResponse(message, context)
    }

    if (hasClaudeAPI) {
      // Use Claude via API route for better streaming support
      return `Procesando con Claude AI... Para una mejor experiencia, usa el chat en tiempo real.`
    }

    // Fallback to Grok if Claude not available
...
export async function streamChatResponse(message: string, conversationHistory: ChatMessage[]) {
  try {
    const context = await getCRMContext()
